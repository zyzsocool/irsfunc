...
class irsfunc2():
    ...
    def interestread(self, interestaddress):
        ...
        # 2读入SHIBOR3M的利率互换曲线和FR007的历史值（待写）

        # 3读入LPR1Y的利率互换曲线和FR007的历史值（待写）
    ...
    def interestline(self, change=0):
        ...
        # 2计算SHIBOR3M的IRS收益率曲线（待写）
        # 3计算LPR1Y的IRS收益率曲线（待写）
        ...
        for code, info in self.asset.items():
            ...
            if info[0] == 'FR007':
                ...
                if output == 0:
                    ...
                    for x in listbb:
                        print(x)
            # 1.2计算SHIBOR3M的IRS的价值（待写）

            # 1.3计算LPR1Y的IRS的价值（待写）
...
b.irsput('C:/Users/zyzse/Desktop/1交易查询与维护.xlsx')
m=b.valuecal()
...
# change=[['FR007','3M',0.0001],['FR007','6M',0.0001],['FR007','9M',0.0001],['FR007','1Y',0.0001]]
